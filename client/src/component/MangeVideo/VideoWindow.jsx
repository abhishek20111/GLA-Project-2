...
const VideoPlayer = ({ videoUrl, isPlaying }) => {
  if(videoUrl === undefined) return <div></div>
  let player;
  useEffect(() => {
    player = cloudinary.videoPlayer("playerID", {
      cloudName: "dsf35gixc",
      fluid: true,
      controls: true,
      fontFace: "Fira Sans",
    ...
    console.log("Cloudinary player initialized ",player);
  }, [videoUrl]);

  return (
    <div className="p-5">
      <div style={{ position: "relative", paddingTop: "56.25%" }}>
        <video id="playerID" controls autoplay class="cld-video-player"></video>
      </div>
    </div>
  );
};
...
const VideoWindow = ({ selectedSyllabus, selectedVideoUrl, courceData }) => {
  ...
  const syllabus =
    selectedSyllabus || (location.state && location.state.selectedSyllabus);
  const videoUrl =
    selectedVideoUrl || (location.state && location.state.selectedVideoUrl);
  const courceDetails =
    courceData || (location.state && location.state.courceData);
  ...
  //       `http://localhost:8080/chat/message/get/${id}`
  //     );
  //     console.log(backData.data);
  //     if (backData.data.length > 0) setMessages(backData.data);
  //   } catch (error) {
  ...
  return (
    <div className="flex flex-col">
      ...
      <div>
        {userId && courceDetails && (
          <Review userId={userId} courseDetails={courceDetails} />
        )}
      </div>
    </div>
  );
